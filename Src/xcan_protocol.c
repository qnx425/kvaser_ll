...
xcan_device =
{
  .can[0] = 
  {
    ...
    /* channel name */
    .sz_name = "Moonglow"	// this string not changed - it is a conscious decision
  },
};
...
{
  xcan_device.can[0].rx_err = rx_err;
  xcan_device.can[0].tx_err = tx_err;
...
#ifdef DEBUG
  if( pcmd->head.cmdNo < 126 ) printf( "%s \r\n", cmdStr[pcmd->head.cmdNo] );
  if( pcmd->head.cmdNo == CMD_GET_CAPABILITIES_REQ ) if( pcmd->capabilitiesReq.subCmdNo < 11 ) printf( "  %s \r\n", capSubCmdStr[pcmd->capabilitiesReq.subCmdNo] );
  if( pcmd->head.cmdNo == CMD_SET_DRIVERMODE_REQ ) if( pcmd->setDrivermodeReq.driverMode < 5 ) printf( "  %s \r\n", driverModeStr[pcmd->setDrivermodeReq.driverMode] );
  if( pcmd->head.cmdNo == CMD_SET_BUSPARAMS_REQ  ) {
      printf( "  bitRate : %d kBit/s \r\n", pcmd->setBusparamsReq.bitRate / 1000 );
      printf( "  tseg1   : %d \r\n", pcmd->setBusparamsReq.tseg1 );
      printf( "  tseg2   : %d \r\n", pcmd->setBusparamsReq.tseg2 );
      printf( "  sjw     : %d \r\n", pcmd->setBusparamsReq.sjw );
      printf( "  noSamp  : %d \r\n", pcmd->setBusparamsReq.noSamp );
  }
#endif
  ...
  switch( pcmd->head.cmdNo )
  {
    case CMD_GET_CARD_INFO_REQ:
    {
      ...
      if( !p_resp )
       ...
       p_resp->serialNumber = 555u;
       p_resp->padding1 = 0;
       p_resp->clockResolution = 0;
       p_resp->mfgDate = 1690517280; /* unix time */
       p_resp->EAN[0] = 0x50;
       p_resp->EAN[1] = 0x68;
       p_resp->EAN[2] = 0x00;
       p_resp->EAN[3] = 0x30;
    ...
    case CMD_GET_CHIP_STATE_REQ:
    {
      ...
      // kvn
      p_resp->busStatus      = xcan_device.can[0].err;
      p_resp->txErrorCounter = xcan_device.can[0].tx_err ;
      p_resp->rxErrorCounter = xcan_device.can[0].rx_err;
    }
    break;
    ...
    case CMD_SET_BUSPARAMS_REQ:
    {
      ...
      // kvn
      xcan_can_set_bitrate( pcmd->setBusparamsReq.bitRate );
      //
      /* save for future use */
    ...
    case CMD_LED_ACTION_REQ:
    {
      ...
      switch( p_resp->subCmd )
      {
        ...
        case LED_SUBCOMMAND_LED_1_OFF:
          xcan_led_set_mode( LED0, ( p_resp->subCmd == LED_SUBCOMMAND_LED_1_ON )?LED_MODE_ON:LED_MODE_OFF
                    , pcmd->ledActionReq.timeout );
        break;
      }
    }
